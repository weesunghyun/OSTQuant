        ...
        for loss in "${losses[@]}"; do
            ...
            if [[ $loss == *"contrastive"* ]]; then
                for contrastive_loss_weight in "${contrastive_loss_weights[@]}"; do
                    ...
                    CUDA_VISIBLE_DEVICES=${gpu_id} torchrun --nproc_per_node=1 --master_port=${master_port} main.py \
                    --output_dir "${output_dir}" --model "${input_model}" --pretrained_model "${pretrained_model}" \
